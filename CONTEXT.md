...
Next:
1. **First-run wizard** (`/setup`): admin user creation + initial library config (uses the existing `setup.complete` server setting).
2. **Self-host pdf.js** so PDF progress can be tracked per page (and so we don't depend on the browser's built-in viewer; this also enables proper "current page" capture for PDF bookmarks instead of the prompt fallback).
3. **Generate the missing migrations** for SQL Server, Postgres, MySQL. SQLite already has its `InitialCreate` migration. To regenerate after model changes:
...
4. **Integration tests** for the auth-mode switch, scanner, reader.
5. **Recurring scans** — schedule each library's scan via Hangfire `RecurringJob` instead of (or in addition to) on-demand triggering.
...
- The reader (`/read/{id}`) loads `epub.js` + `jszip` lazily from `wwwroot/lib/{epubjs,jszip}/dist/` (self-hosted; tracked in `libman.json`) and renders into `#epub-viewer`. PDFs are an iframe against `/files/{id}` — file streaming goes through `BookFilesController` with `enableRangeProcessing: true`. Progress flows: JS `relocated` event → `DotNetObjectReference.OnProgress(percent, page, cfi)` → `IBookService.SaveProgressAsync` → `BookProgress` table. Bookmarks flow through `IBookmarkService` and reuse the same CFI / page coordinate space (`shelfwardenReader.getEpubLocation` / `gotoEpub` / `gotoPdfPage`).
