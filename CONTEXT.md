...
| `Universe` / `UniverseBook` | Fictional universe grouping series + books. `Series.UniverseId` is a nullable FK; book membership lives on the `UniverseBook` join entity along with `TimelineDate` (free text, nvarchar(50)) and `TimelineOrder` (int). |
...
- ✅ **Timeline view vs edit** — The Timeline tab defaults to `Components/Shared/UniverseTimeline.razor`, a read-only horizontal rail of covers hanging off a single axis with the free-text date as a pill above each one, colour-coded per series (hue derived from a hash of the series name, so it's stable without a stored palette). "Edit timeline" swaps in the previous `.reading-list`-style editor. **Positions along the rail come from `TimelineOrder`, evenly spaced** — nothing parses `TimelineDate`, so this is a sequence, not a scaled axis.
