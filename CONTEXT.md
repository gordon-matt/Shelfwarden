...
Next:
1. **Collections + Reading Lists UI.**
2. **First-run wizard** (`/setup`): admin user creation + initial library config (uses the existing `setup.complete` server setting).
3. **Self-host pdf.js** so PDF progress can be tracked per page (and so we don't depend on the browser's built-in viewer; this also enables proper "current page" capture for PDF bookmarks instead of the prompt fallback).
