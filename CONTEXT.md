...
| `Universe` / `UniverseBook` / `TimelineDate` | Fictional universe grouping series + books. `Series.UniverseId` is a nullable FK; book membership lives on the `UniverseBook` join entity. `TimelineDate` is a universe-owned, admin-maintained point on the timeline (`Date` free text nvarchar(50), `Order` int); `UniverseBook.TimelineDateId` is nullable (null = unscheduled, FK `SetNull`) and `UniverseBook.Order` sequences the books sharing one date. |
...
- ✅ **Timeline view vs edit** — The Timeline tab defaults to `Components/Shared/UniverseTimeline.razor`, a read-only matrix: one column per `TimelineDate` (plus a trailing "Unscheduled" column while anything needs placing) crossed with one row per series (plus a shared "Standalone" lane), so a series' books stay in that series' lane instead of pooling into one strip. Lanes are colour-coded per series (hue derived from a hash of the series name, so it's stable without a stored palette) and empty cells keep a dotted rail so a lane reads as continuous. "Edit timeline" swaps in a `.reading-list`-style editor grouped the same way: one card per date, drag the card to move the date, drag a book to move it within its date, change a book's date from a dropdown. **Columns are evenly spaced and ordered by `TimelineDate.Order`** — nothing parses the date text, so this is a sequence, not a scaled axis.
