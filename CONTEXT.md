...
| `Universe` / `UniverseBook` / `TimelineDate` | Fictional universe grouping series + books. `Series.UniverseId` is a nullable FK; book membership lives on the `UniverseBook` join entity. `Universe.TimelineType` (`Named` default / `Numeric`) picks how the universe's dates are interpreted — see below. `TimelineDate` is a universe-owned, admin-maintained point on the timeline (`Date` free text nvarchar(50) — auto-derived from the years when the universe is `Numeric`, `Order` int, optional `YearFrom`/`YearTo` nullable ints); `UniverseBook.TimelineDateId` is nullable (null = unscheduled, FK `SetNull`) and `UniverseBook.Order` sequences the books sharing one date. |
...
The web host registers three Hangfire servers — `main` (default + critical queues), `scan` (dedicated single-worker for library scans), and `tts` (dedicated single-worker for audiobook generation). Each long-running queue gets its own server so a slow scan or audiobook never blocks the default queue.
...
- ✅ `Shelfwarden.Data.Sql` / `Data.Npgsql` / `Data.MySql` (concrete DbContext + factory + DI extension + `InitialCreate` migration each). MySQL uses Oracle's `MySql.EntityFrameworkCore` (Pomelo has no EF Core 10 build) and strips the shared `app` schema in `OnModelCreating` since MySQL treats a schema as a database.
...
- ✅ **Provider migrations generated** — `InitialCreate` migrations now exist for `Shelfwarden.Data.Sql` (SQL Server), `Shelfwarden.Data.Npgsql` (PostgreSQL) and `Shelfwarden.Data.MySql` (MySQL/MariaDB, via Oracle's `MySql.EntityFrameworkCore`) alongside the existing SQLite snapshot. **MySQL note:** the shared `HasDefaultSchema("app")` is stripped in `Shelfwarden.Data.MySql.ApplicationDbContext.OnModelCreating` because MySQL has no schema concept separate from the database; Hangfire uses `Hangfire.Storage.MySql` (connection strings should include `Allow User Variables=True`). To add migrations for a provider: set `Database:Provider` to that provider in `appsettings.Development.json` first so the `--startup-project` resolves the right factory, then run `dotnet ef migrations add <Name> -p Shelfwarden.Data.<Provider> -s Shelfwarden -c Shelfwarden.Data.<Provider>.ApplicationDbContext --output-dir Migrations`. EF 10.0.6+ tooling needs `Microsoft.EntityFrameworkCore.Design` on the **startup project** (`Shelfwarden`) too, otherwise `dotnet ef` fails with `MissingMethodException: AbstractionsStrings.ArgumentIsEmpty(Object)` (see [efcore#38107](https://github.com/dotnet/efcore/issues/38107)).
- ✅ **Text-to-speech / audiobook generation** — End-to-end `Book → .m4a` pipeline driven by [KokoroSharp.CPU](https://www.nuget.org/packages/KokoroSharp.CPU) for ONNX-based neural TTS and [Xabe.FFmpeg](https://www.nuget.org/packages/Xabe.FFmpeg) for AAC encoding. Backed by the new `Audiobook` entity (one row per book; tracks status / voice / chunk progress / output filename / error message), a dedicated Hangfire `tts` queue + worker, and an `IAudiobookService` API surface for the UI. Pipeline lives in `Shelfwarden.Services/Tts/`: `IBookTextExtractor` (Epub via VersOne, PDF via PdfPig) streams paragraphs → `TextChunker` (sentence-split + ~250 char chunking) → `KokoroEngineProvider` (lazy ONNX model loader; downloads to `<TtsCacheDirectory>/kokoro.onnx` on first use) synthesises one chunk at a time → `AudioStitcher` writes raw PCM chunk files to `<AudiobooksDirectory>/_tmp/{bookId}/chunk_NNNN.pcm` (resumable — already-present chunks get skipped on a retry) → `FFmpegProvider` ensures the FFmpeg binary is present (Xabe downloader, cached under `<TtsCacheDirectory>/ffmpeg`) and `AudioStitcher.EncodeAsync` concatenates the PCMs into a single `.m4a` (`-c:a aac -b:a 64k -movflags +faststart`). UI: `BookDetail.razor` shows a "Generate Audio" button (replaced by a "Listen" button + `<audio>` player when complete); the polling timer refreshes status every 3 s so navigating away and coming back surfaces live progress. Voice selection happens through `Components/Shared/VoicePickerModal.razor`, which lists all loaded Kokoro voices grouped by language with a per-voice play preview that hits `audiobooks/voices/{name}/preview` (cached short WAV synthesis, lives under `<AudiobooksDirectory>/_voicesamples/`). Streaming: `AudiobooksController` serves `/audiobooks/{bookId}` (range-enabled) and the voice previews — both auth-required. **Caveats:** the first audiobook generation OR voice preview triggers a one-time ~320 MB download of the Kokoro float32 model and ~70 MB of FFmpeg binaries; both land under `<TtsCacheDirectory>` (defaults to `tts-cache/` next to `covers/`) so they survive container restarts. Phonemisation goes through eSpeak-NG, which the `KokoroSharp.CPU` package copies to `bin/.../espeak/` automatically. Xabe's downloader needs network access; in air-gapped scenarios drop a static `ffmpeg` binary into `<TtsCacheDirectory>/ffmpeg/` ahead of time.
- ✅ **Online metadata providers** — Pluggable book-metadata lookup against public APIs, modelled on Calibre's parallel identify/merge pipeline and Booklore's `BookParser` interface. `Shelfwarden.Services/Metadata/`: `IBookMetadataProvider` (stateless singletons using `IHttpClientFactory`) with two implementations — `GoogleBooksMetadataProvider` (official `googleapis.com/books/v1`, no key) and `OpenLibraryMetadataProvider` (`openlibrary.org/search.json` + per-work description hydration). `IBookMetadataService` (`BookMetadataService`, scoped — admin-gated `SearchAsync` needs `IUserContextService`) fans queries out to every provider in parallel, ranks candidates (ISBN match → title similarity → field completeness → provider priority) and can `FindBestMatchAsync` (top result with null fields back-filled from lower-ranked results). DTOs live in `Shelfwarden.Models/Metadata/` (`ExternalBookMetadataDto`, `BookMetadataSearchRequest`). **Two entry points:** (1) a "Fetch metadata" button on `BookEdit.razor` opens `Components/Shared/BookMetadataModal.razor` (search by title/author/ISBN → pick a candidate → it populates the edit form client-side; nothing is saved until the user hits Save — covers are intentionally left untouched). (2) A per-shelf `Shelf.AutoFetchOnlineMetadata` flag (surfaced in `ShelfEdit.razor`) makes `ScannerService` back-fill *only the empty* metadata fields from online sources during import (best-effort, respects the shelf's "always ignore author/tags/genres" flags). Amazon/Goodreads were deliberately skipped (HTML scraping, brittle/ToS). Migration: `AddShelfAutoFetchMetadata` (Sqlite + Sql + Npgsql). New `IBookMetadataProvider` registrations automatically join the search/merge. Tests: `MetadataNormalizationTests`, `BookMetadataServiceTests`.
- ✅ **Audiobook chapter detection + front-matter skipping** — Before generating, the admin reviews a **section editor** (`Components/Shared/SectionEditor.razor`) listing the book's parts. Sections come from `IEbookSectionParser` (`Shelfwarden.Services/Tts/`): `EpubSectionParser` builds them from the EPUB navigation (each top-level ToC entry spans the reading-order documents up to the next), `PdfSectionParser` uses a tiered strategy (PdfPig bookmarks → large-top-of-page heading heuristics → single whole-document fallback), surfaced to the UI via `SectionDetectionResult` (`Sections` + `Quality` + `Warning`). `SectionClassifier` keyword-matches titles to auto-detect front matter (copyright, ToC, dedication, …) and back matter (index, bibliography, …) and unticks them by default, so copyright/ToC pages are skipped without manual fiddling. The editor also has a **"split into one file per chapter"** switch with per-section "new file" boundary toggles. The reviewed plan is persisted on the `Audiobook` row as `SectionPlanJson` (+ `SplitByChapter`); `TtsJobService` replays it, grouping included sections into render units (one unit per chapter when split) and encoding each to its own `.m4a` (`<AudiobooksDirectory>/{bookId}/{index}.m4a`), with chapter-level resume (an existing chapter file is skipped on retry). Finished chapters are recorded in `ChaptersJson` and surfaced through `AudiobookDto.Chapters`. `AudiobooksController` gained `/audiobooks/{bookId}/chapters/{index}` (range-enabled per-chapter playback) and `/audiobooks/{bookId}/zip` (streams all files as a ZIP, members stored uncompressed since AAC is already compressed). The old whole-book `IBookTextExtractor` stack was replaced by the section parsers; a null `SectionPlanJson` still means "whole book" for backward-compatible single-file generation. Migration: `AddAudiobookSectioning` (Sqlite + Sql + Npgsql).

- ✅ **Universes** — `Universe` + `UniverseBook` entities plus nullable `Series.UniverseId` / `ReadingList.UniverseId` (migration `AddUniverse` across all four providers). `IUniverseService` follows the `ISeriesService` split: reads are open to any signed-in user, every mutation is administrator-only. Deliberate design choices: (1) **no `UniverseReadingList` junction** — `ReadingList.UniverseId` already pins a list to exactly one universe, and two sources of truth would drift; (2) universe reading orders are owned by `Constants.GlobalUserId` so everyone browsing the universe can follow them, which is why `ReadingListService` gained `CanRead` (owner *or* global) / `CanModify` (owner, or global + admin) helpers and why the `ReadingLists` unique index is now `(OwnerUserId, UniverseId, Name)`; (3) `ReadingListService.ListAsync` filters `UniverseId == null` so universe orders never leak into `/reading-lists`, the sidebar or the "Add to…" dropdown. Series ↔ book membership is never auto-synced — `UniverseBook` is the authoritative answer to "is this book in this universe" — though the series edit modal offers a one-shot "also add this series' books" checkbox. Timeline ordering comes from `TimelineDate.Order` then `UniverseBook.Order`; the date text is free text and is never parsed. UI: `/universes` (tile list, `SeriesCollage` covers) and `/universes/{id}` with C#-driven tabs (Overview / Series / Books / Reading orders / Timeline); the timeline editor reuses the `.reading-list` CSS and the same "send the full new ordering" reorder contract as reading lists. Universe pickers were added to `BookEdit.razor` (universe + timeline date, saved through `SetBookMembershipAsync` *after* `BookService.UpdateAsync`) and to the series edit modal on `SeriesIndex.razor`. Tests: `UniverseServiceTests`.
- ✅ **Universe reading orders are scoped to the universe** — A new order is created pre-filled with the entire timeline (`UniverseService.CreateReadingListAsync` inserts one `ReadingListItem` per `UniverseBook`), because an "order" is a re-arrangement of the universe rather than a subset assembled book by book. `ReadingListService` enforces the boundary from then on: `AddBookAsync` returns an error for a book outside the universe and `AddBooksAsync` silently drops them, while `BookSearchRequest.UniverseId` keeps the detail page's add-a-book search inside the universe. Non-administrators see universe orders read-only (`ReadingListDetail.CanModify`).
- ✅ **Shared "Add to…" modal** — `Components/Layout/AddToPicker.razor` (hosted once in `MainLayout`, opened via cascading parameter from `Components/Shared/AddToActions.razor`) replaces the old `ReadingListPicker` / `AddToReadingListAction` pair and the inline collection/list dropdown on `BookDetail.razor`. One dialog covers three targets (`AddToTarget` = Collection / ReadingList / Universe) for either a book or a whole series, with inline create-and-add. Enumerating targets in a context menu didn't survive contact with a real library. Series → universe assigns `Series.UniverseId` (with an optional "also add its books" tick) rather than appending, since a series belongs to at most one universe. Global collections are hidden from non-administrators, and "Add to universe" is administrator-only. `ICollectionService.AddSeriesAsync` was added to match the reading-list equivalent.
- ✅ **Timeline view vs edit** — The Timeline tab defaults to `Components/Shared/UniverseTimeline.razor`, a read-only view: one row per series (plus a shared "Standalone" lane), so a series' books stay in that series' lane instead of pooling into one strip. Lanes are colour-coded per series (hue derived from a hash of the series name, so it's stable without a stored palette). Book covers never render inline on the chart — clicking a lane's own name opens its series page (or, for "Standalone", a covers modal, since there's no page to link to); clicking one of its bars opens a covers modal for just the books on that bar, each linking to its book detail page. "Edit timeline" swaps in a `.reading-list`-style editor grouped by date: arrows move a date card up/down the timeline, drag a book to move it within its date, change a book's date from a dropdown, and a modal (reachable from both the read-only view's toolbar and `BookEdit.razor`) creates/renames dates.
- ✅ **`Universe.TimelineType` (Named / Numeric) drives the axis, not a heuristic** — Defaults to `Named` (free-text dates, hand-ordered, evenly-spaced columns). Creating or renaming a `TimelineDate` with a `YearFrom`/`YearTo` (`UniverseService.CreateTimelineDateAsync`/`RenameTimelineDateAsync`, both take a nullable `date`) automatically flips the owning universe to `Numeric`; supplying `date` text with no years flips it back to `Named` — this is the server-side effect of the two-radio-button choice on the "new/rename date" modal (`UniverseDetail.razor` and `BookEdit.razor`), which shows only the `Date` field for `Named` or only the year fields for `Numeric`. When `Numeric`, `Date` is auto-derived from the years (`UniverseService.FormatYearRange`, e.g. `"2005-2008"`) purely as a fallback label for dropdowns — the read-only chart ignores it. `Numeric` switches `UniverseTimeline.razor` from evenly-spaced columns to a proportional axis (with ruler tick marks) built from those years, so bars from two *different* dates with overlapping ranges visibly overlap across lanes. Dates without a year (a stray `Named`-era leftover) trail the axis in fixed-width slots, same treatment as the "Unscheduled" bucket; a dashed divider plus a wide gap keeps that trailing area from crowding the last year tick. Ordering follows `Universe.TimelineType` everywhere dates are read (`UniverseService.OrderTimelineDates(dates, timelineType)`): `Numeric` sorts `OrderBy(YearFrom ?? YearTo ?? int.MaxValue).ThenBy(YearTo ?? YearFrom ?? int.MaxValue).ThenBy(Order)` (plain integer comparison, so BCE-style negative years sort correctly), `Named` sorts `OrderBy(Order)`.
- ✅ **Overlapping numeric dates on the same lane merge into one bar** — `UniverseService.BuildSegments` (called from `LoadTimelineAsync`, exposed as `UniverseTimelineRowDto.Segments`) walks a lane's occupied numeric dates sorted by `YearFrom`, and merges any whose range overlaps or touches the previous one (`next.Start <= current.End`) into a single segment spanning the combined range and books, labelled from the merged years (e.g. dates "2005-2008" and "2007-2012" on the same lane become one "2005-2012" bar) — a genuine gap keeps them as separate bars. This runs per lane, not universe-wide, so the same two dates can merge on one series' lane while staying separate on another's. `UniverseTimeline.razor` positions `Segments` directly; it no longer computes merging itself. Lanes (rows) sort by their earliest merged-segment year, then name, when `Numeric`; otherwise by first-occupied-column then name, as before.
- ✅ **Timeline dates are first-class records, not typed-in text** — `TimelineDate` replaced the per-book free-text date + integer order. Dates are created, renamed, reordered and deleted explicitly (duplicate text within a universe is rejected, since two identical entries would be indistinguishable in a dropdown), and books are assigned to one from a list. Books join a universe **unscheduled** rather than having a date guessed for them, and deleting a date un-schedules its books instead of evicting them from the universe. This replaced an earlier scheme that inferred shared "slots" by matching date text between adjacent entries — merging and splitting on every keystroke turned out to be both surprising and impossible to reorder predictably. Migrations `AddTimelineDateAssignment` (schema) + `BackfillTimelineDateAssignment` (data) across all four providers; they're split because SQLite applies the column changes by rebuilding the table at the *end* of a migration, so data statements alongside them would run against the old definition.
- ✅ **Drag-and-drop reordering** — Reading lists and the timeline editor use native HTML5 drag events (no JS interop, no library) coordinated by `Components/Shared/ListReorderState.cs`, which tracks the source/target row and produces the new id ordering. Only the grip handle is `draggable` so row text stays selectable. The old up/down arrows became "move to top" / "move to bottom", which covers the long-list case the arrows were bad at.
- ✅ **Tests run on real SQLite** — `Shelfwarden.Tests/Infrastructure/TestDbFixture.cs` (was `InMemoryDbFixture`) points `AddShelfwardenSqlite` at a shared-cache in-memory SQLite database, holding one connection open for the fixture's lifetime so SQLite doesn't drop the database between pooled connections. This buys real foreign keys, real unique indexes and `ExecuteUpdate` / `ExecuteDelete` support, and it matches the provider most installs actually run. Bulk operations were subsequently restored in `UniverseService`, `ReadingListService`, `TagService`, `GenreService` and `AdditionalContentTagService` wherever rows were being loaded only to be deleted or uniformly updated.
